...
int main() {
	...
	for (int i = 0; i < SIZE; i++) {
		cin >> mas[i];          //введення елементів масиву з клавіатури
	for (int i = 0; i < SIZE; i++)
		cout << "mas[" << i << "]=" << mas[i] << endl; //виведення елементів масиву
	int summa = 0;
	for (int i = 0; i < SIZE; i++) {
		if (mas[i] < 0)
			summa = summa + mas[i];    //знаходження суми від'ємних елементів
	}
	cout << "Сума:" << summa << endl;
	}
